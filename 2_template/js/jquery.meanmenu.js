/*!
 * jQuery meanMenu v2.0.6.custom-2
...
(function ($) {
    ...
    $.fn.meanmenu = function (options) {
        var defaults = {
            ...
            meanScreenWidth: "890", // set the screen width you want meanmenu to kick in at
...
// ------------------- --------  tova e za kontaktite
	...
	if (isIE11) {
		...
		if(!target.is('.contacts-c-b') && !target.is('.contacts-c-b *') && !target.is('#header-buttons') && !target.is('#header-buttons *') && !target.is('#top-lang') && !target.is('#top-lang *') && !target.is('.select2-container') && !target.is('.select2-container *') && !target.is('.select2-choice') && !target.is('.select2-choice *') && !target.is('.select2-chosen') && !target.is('.select2-chosen *') ) {
			if ( $('#header-buttons.sh').is(':visible') ) {
				$('#header-buttons.sh').hide();
			}
		}
	}
/*   	if( !target.is('.mean-nav') &&!target.is('.mean-nav *') && !target.is('.meanmenu-reveal')  && !target.is('.mtxt') &&  !target.is('.micn') &&  !target.is('.micn *')) {
		if( menuOn == true ) {
			jQuery('.mean-nav ul:first').slideUp();
			menuOn = false;   
			$navreveal.toggleClass("meanclose");
